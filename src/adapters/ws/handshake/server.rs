        ...
        Ok(match finish {
            ...
            StageResult::DoneWriting(stream) => {
                if let Some(err) = self.error_response.take() {
                    ...
                    let body_opt = body.map(|b| {
                        //
                        let mut body_buffer = take_packet(b.len());
                        body_buffer.append_slice(b.as_bytes());
                        body_buffer
                    });
                    return Err(Error::Http(http::Response::from_parts(parts, body_opt)));
                } else {
