...
pub fn connect_with_config<Req: IntoClientRequest>(
    ...
    max_redirects: u8,
) -> Result<(WebSocket<WsStream<TcpStream>>, Response)> {
    fn try_client_handshake(
        ...
        let client = ws_connect_with_config(request, WsStream::Plain(stream), config);
...
pub fn ws_connect_with_config<Stream, Req>(
    request: Req,
    stream: Stream,
    config: Option<WebSocketConfig>,
...
where
    ...
    Req: IntoClientRequest,
{
    ws_connect_with_config(request, stream, None)
}
...
impl<'h, 'b> IntoClientRequest for httparse::Request<'h, 'b> {
    fn into_client_request(self) -> Result<Request> {
        use super::handshake::headers::FromHttparse;
