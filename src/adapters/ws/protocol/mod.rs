...
impl WebSocketContext {
        ...
        Stream: Read + Write,
    {
        ...
        let frame = match message {
            Message::Text(data) => {
                //
                let mut buffer = take_packet(data.len());
                buffer.append_slice(data.as_bytes());
                Frame::message(buffer, OpCode::Data(OpData::Text), true)
            },
            Message::Binary(buffer) => {
                //
                Frame::message(buffer, OpCode::Data(OpData::Binary), true)
            },
            Message::Ping(buffer) =>{
                //
                Frame::ping(buffer)
            ...
            Message::Pong(buffer) => {
                self.set_additional(Frame::pong(buffer));
        ...
        Stream: Read + Write,
    {
        if let Some(mut frame) =
            self.frame.read_frame(stream, self.config.max_frame_size).check_connection_reset(self.state)?
        {
            ...
            match frame.header().opcode {
                OpCode::Control(ctl) => {
                    match ctl {
                        ...
                        _ if frame.payload().buffer_raw_len() > 125 => {
...
mod tests {
    ...
    use crate::adapters::ws::error::{CapacityError, Error};
