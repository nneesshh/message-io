...
use crate::adapters::ws::protocol::Message;

use log::*;
use net_packet::{take_small_packet, NetPacketGuard, take_packet};
...
pub(super) struct FrameCodec {
    ...
    in_buffer_opt: Option<NetPacketGuard>,
    /// Buffer to send packets to the network.
    out_buffer_opt: Option<NetPacketGuard>,
...
impl FrameCodec {
    ...
    pub(super) fn new() -> Self {
        Self {
            in_buffer_opt: Some(take_small_packet()),
            out_buffer_opt: Some(take_small_packet()),
    ...
    pub(super) fn from_partially_read(part: NetPacketGuard) -> Self {
        Self {
            in_buffer_opt: Some(part),
            out_buffer_opt: Some(take_small_packet()),
            max_out_buffer_len: usize::MAX,
            out_buffer_write_len: 0,
            header: None,
        ...
        let payload = loop {
            {
                ...
                if let Some((_, ref length)) = self.header {
                    ...
                    if length <= input_size {
                        ...
                        let mut payload = take_packet(length as usize);
                        if length > 0 {
                            let buffer = payload.as_write_mut();
                            cursor.take(length).read_to_end(buffer)?;
                        }
                        break payload;
                    }
                }
...
impl std::fmt::Debug for FrameCodec {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let string =
            format!("(out_buffer: {}/{})", self.out_buffer_write_len, self.max_out_buffer_len);
        write!(f, "FrameCodec::{string}")
    }
}
...
mod tests {
    ...
    impl<Stream> FrameSocket<Stream> {
        ...
        pub fn from_partially_read(stream: Stream, part: NetPacketGuard) -> Self {
            FrameSocket { stream, codec: FrameCodec::from_partially_read(part) }
        }

        /// Extract a stream from the socket.
        pub fn into_inner(self) -> (Stream, NetPacketGuard) {
    ...
    fn read_frames() {
        ...
        assert_eq!(rest.peek(), vec![0x99]);
    }

    #[test]
    fn from_partially_read() {
        let raw = Cursor::new(vec![0x02, 0x03, 0x04, 0x05, 0x06, 0x07]);
        let part_vec = vec![0x82, 0x07, 0x01];
        let mut part = take_small_packet();
        part.append_slice(part_vec.as_slice());
        let mut sock = FrameSocket::from_partially_read(raw, part);
        assert_eq!(
            sock.read(None).unwrap().unwrap().into_data(),
            vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07]
        );
