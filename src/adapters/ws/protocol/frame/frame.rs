...
use log::*;
use net_packet::{NetPacketGuard, take_packet};
use std::{
    default::Default,
    fmt,
...
use crate::adapters::ws::error::{Error, ProtocolError, Result};

/// A struct representing the close command.
pub struct CloseFrame {
    /// The reason as a code.
    pub code: CloseCode,
    /// The reason text buffer.
    pub reason: NetPacketGuard,
}

impl fmt::Display for CloseFrame {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let reason = unsafe { std::str::from_utf8_unchecked(self.reason.peek())};
        write!(f, "{} ({})", reason, self.code)
    }
}

impl fmt::Debug for CloseFrame {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let reason = unsafe { std::str::from_utf8_unchecked(self.reason.peek())};
        let string = std::format!("{} ({})", reason, self.code);
        write!(f, "CloseFrame::{string}")
    }
}

impl std::clone::Clone for CloseFrame {
    fn clone(&self) -> Self {
        let mut buffer = take_packet(self.reason.buffer_raw_len());
        buffer.append_slice(self.reason.peek());
...
impl Frame {
    ...
    pub fn len(&self) -> usize {
        let length = self.payload.buffer_raw_len();
    ...
    #[inline]
    pub fn payload(&self) -> &NetPacketGuard {
        &self.payload
    }

    /// Get a mutable reference to the frame's payload.
    #[inline]
    pub fn payload_mut(&mut self) -> &mut NetPacketGuard {
        &mut self.payload
    }
    ...
    pub(crate) fn apply_mask(&mut self) {
        if let Some(mask) = self.header.mask.take() {
            let cursor = self.payload.as_cursor_mut();
            apply_mask(cursor.get_mut(), mask)
        }
    }

    /// Consume the frame into its payload as binary.
    #[inline]
    pub fn into_buffer(self) -> NetPacketGuard {
        self.payload
    }
    ...
    #[inline]
    pub fn message(payload: NetPacketGuard, opcode: OpCode, is_final: bool) -> Frame {
        debug_assert!(matches!(opcode, OpCode::Data(_)), "Invalid opcode for data frame.");

        Frame { header: FrameHeader { is_final, opcode, ..FrameHeader::default() }, payload }
    }

    /// Create a new Pong control frame.
    #[inline]
    pub fn pong(payload: NetPacketGuard) -> Frame {
        Frame {
            header: FrameHeader {
    ...
    #[inline]
    pub fn ping(payload: NetPacketGuard) -> Frame {
        Frame {
            header: FrameHeader {
    ...
    #[inline]
    pub fn close(msg: Option<CloseFrame>) -> Option<Frame> {
        if let Some(CloseFrame { code, reason }) = msg {            
            let mut payload = reason;
            payload.prepend_u16(code.into());
            Some(Frame { header: FrameHeader::default(), payload })
        } else {
            None
        }
    }
    ...
    pub fn from_payload(header: FrameHeader, payload: NetPacketGuard) -> Self {
        Frame { header, payload }
    }

    /// Write a frame out to a buffer
    pub fn format(mut self, output: &mut impl Write) -> Result<()> {
        self.header.format(self.payload.buffer_raw_len() as u64, output)?;
        self.apply_mask();
        output.write_all(self.payload().peek())?;
        Ok(())
    }
}
...
payload: 0x{}
            ...
            self.len(),
            self.payload.buffer_raw_len(),
            self.payload.peek().iter().map(|byte| format!("{:02x}", byte)).collect::<String>()
        )
    }
}
...
impl fmt::Debug for Frame {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let string = std::format!("{}", self.payload.buffer_raw_len());
        write!(f, "Frame::{string}")
    }
}
