...
pub use super::handshake::server::ServerHandshake;
...
use super::protocol::{WebSocket, WebSocketConfig};
...
pub fn ws_accept_with_config<S: Read + Write>(
    stream: S,
    config: Option<WebSocketConfig>,
) -> Result<WebSocket<S>, HandshakeError<ServerHandshake<S, NoCallback>>> {
    ws_accept_hdr_with_config(stream, NoCallback, config)
}
...
/// `openssl::ssl::SslStream` for the stream here. Any `Read + Write` streams are supported,
/// including those from `Mio` and others.
pub fn ws_accept<S: Read + Write>(
    stream: S,
) -> Result<WebSocket<S>, HandshakeError<ServerHandshake<S, NoCallback>>> {
    ws_accept_with_config(stream, None)
}
...
pub fn ws_accept_hdr_with_config<S: Read + Write, C: Callback>(
    stream: S,
    callback: C,
    config: Option<WebSocketConfig>,
...
/// for header processing. The callback receives headers of the incoming
/// requests and is able to add extra headers to the reply.
pub fn ws_accept_hdr<S: Read + Write, C: Callback>(
    stream: S,
    callback: C,
) -> Result<WebSocket<S>, HandshakeError<ServerHandshake<S, C>>> {
    ws_accept_hdr_with_config(stream, callback, None)
}
